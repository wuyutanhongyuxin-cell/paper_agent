...
### Added (M-E Red team + Ensemble — spec §D.4 / 0.1.1 路线图 M-E)
- `paper_agent.score.red_team` — 红队反诘 prompt 池 (M-E.1)，把 PaperOrchestra MIT red-team contract 中 **7 个 generic failure modes** 固化为 paper-agnostic 子模块：`factual_fabrication` (ERROR) / `methodology_handwave` (WARN) / `selective_reporting` (WARN) / `citation_inflation` (WARN) / `ethics_omission` (ERROR) / `stat_misuse` (WARN) / `scope_overclaim` (WARN, 含 T5 社工诱导词 fast-path regex). `RedTeamCheck` (frozen) 含 `detector_regex` 可选字段：有 regex 的 mode 不依赖 LLM 即可命中 obvious 案例 (e.g., `scope_overclaim` 检 `obviously / clearly demonstrates / 显然/毫无疑问`)，detector=None 的 modes 通过 `evaluator` hook 由 M-F 真 LLM 后端注入. `SAFE_REVISION_RE` 白名单 (`(replace|delete|reword) (phrase|sentence|clause): ...`) 让 edit_gate Layer 3 能区分"安全可应用 revision"vs"需人工评估"的改动. CLI `python -m paper_agent.score.red_team --tex paper.tex --paper-id X --modes ... --out ....json`. **33 个单元测试**覆盖 7-mode frozen contract / CHECKS registry 完整性 / detector_regex 命中 + 中性文本不假阳 / mode filter / unknown mode raises / **L-033 read-only (mtime + content 反推)** / evaluator hook 调用契约 / safe_revision 5 真 5 假 / non_aphasia_cs fixture / CLI smoke.
- `paper_agent.score.ensemble` — `num_reviews_ensemble` + Krippendorff α + Area Chair aggregate (M-E.2，spec §D.4): `krippendorff_alpha_interval(units)` 纯数学 interval-level α (Hayes & Krippendorff 2007, 跨学科可消费); `ensemble_alpha(runs)` 把 n 个 `Scored` 映射到 self-reliability α (把 7 维当 units, 每 run 各贡献 1 rating); `num_reviews_ensemble(evaluator, paper_path, *, n_runs, paper_id)` 反复调用 pluggable `EvaluatorFn(paper_path, paper_id, run_idx) -> Scored` 收集 n 个 run; `area_chair_aggregate(runs, paper_id, scored_at)` 取每维 median 作终值，自动算并标注 α，跨 run 的 evidence 按 `(file, line)` 去重保 first-seen 序，red_team_findings 合并. CLI `python -m paper_agent.score.ensemble --runs-dir out/ --paper-id X --out aggregate.json`. **29 个单元测试**覆盖 α 6 种数学边界 (perfect / high-variance / empty / single-rating / Do=De=0 / mild-variance) / ensemble_alpha 4 种 / num_reviews_ensemble 5 种 (n_times / run_idx 顺序 / paper_id 传递 / n=0 / n<0) / Area Chair 10 种 (median 奇偶 / 7-dim contract / ensemble_runs 字段 / α 自动算 / run_scores 合并 / evidence dedup / 空 raises / paper_id override / red_team_findings 合并) / non_aphasia_cs fixture / **L-033 read-only (paper.tex 不被改)** / CLI 2 种.
- `score/__init__.py` 扩 `__all__`：新增 `CHECKS / DEFAULT_FAILURE_MODES / RedTeamCheck / RedTeamFinding / SAFE_REVISION_RE / is_safe_revision / run_red_team / area_chair_aggregate / ensemble_alpha / krippendorff_alpha_interval / num_reviews_ensemble`，对外稳定 API.
- L-033 read-only 合规：M-E 全模块**永不读写** `paper.tex` (red_team 只读, ensemble 只接受 evaluator 返回的 `Scored` 内存对象). 通用化合规 ([[feedback_paper_agent_long_term_generality]])：所有 paper_id 字段 CLI 参数化；α 是纯数学跨学科可消费；红队 prompt 模板全 generic (英文 + 中文社工词均不 hardcode 任何学科 / 论文名); evaluator 注入接口让真 LLM 后端在 M-F 平滑接入而不动 M-E 代码.

### Added (M-D Score core — spec §D.3 / §D.4 / 0.1.1 路线图 M-D)
- `paper_agent.score` 新增子包 —— **7 维评分 schema + halt 机制**（spec §C.2 done criteria #7 整段引入 PaperOrchestra MIT `score_delta.py` 语义）。M-D 不接 LLM，仅落 schema + 算法；M-E `red_team` / `ensemble` 再接 evaluator + Krippendorff α。
- `score/dimensions.py` — `DIMENSIONS` 7 元 frozen tuple (`rigor / novelty / clarity / reproducibility / related / significance / ethics`, spec §D.3 顺序锁死); `EvidenceRef` (frozen) + `DimensionScore` + `Scored` dataclass; `score_tier()` 7 档 anti-inflation 区间锚 (`0-20 fatally / 21-40 substantially / 41-55 issues / 56-70 acceptable / 71-85 strong / 86-92 excellent / 93-100 exceptional`); `derive_confidence()` Krippendorff α 三段 (`high ≥ 0.80 / low ≥ 0.67 / fail`, Rating Roulette EMNLP 2025 arXiv:2510.27106); `Scored.to_json/from_json` schema-lock round-trip（缺/多 dim 抛 ValueError）; CLI `python -m paper_agent.score.dimensions --dump-template --paper-id X --out Y`. 39 个单元测试覆盖 frozen contract / 7 dim 全填 / 8 个 tier 边界 / 4 个 confidence 区间 / 中文 reasoning UTF-8 / evidence 反序列化保 EvidenceRef 类型 / paper-agnostic paper_id 注入 / non_aphasia_cs fixture 验证.
- `score/halt.py` — `HaltDecision` (frozen) + `aggregate_score` + `score_delta` (逐维 + total abs delta) + `detect_halt(history, iter_cap=3, plateau_streak=3, plateau_threshold=1.0, user_signal_halt, user_signal_converged)`. 5 个 exit code: `0=converged/running` `1=iter-cap` `2=plateau` `3=error` `4=halt-by-user`; 优先级 `error > halt-by-user > converged > iter-cap > plateau > running` (iter-cap 优先于 plateau 防 budget 失控). CLI `python -m paper_agent.score.halt --history out/ --iter-cap 3 ...` 返回 exit_code. 18 个单元测试覆盖 score_delta 4 种 / aggregate_score / 5 种 exit_code / iter_cap=0 / streak 边界 / strict-less threshold / HaltDecision frozen.
- `score/prompts/scoring_anchors.md` — 静态 ~80 行：7 档 anti-inflation 区间锚 + LLM prompt contract (必先选区间再给分) + 7 维 generic meaning + α 三段 confidence 表. **完全 paper-agnostic** (无学科 / 语言特异术语), 直接服务 M-E `ensemble.py` prompt 构造.
- `score/__init__.py` — 暴露 `DIMENSIONS / DimensionScore / EvidenceRef / Scored / score_tier / derive_confidence / HaltDecision / detect_halt / score_delta`，对外稳定 API.
- L-033 read-only 合规：score/ 子包**永不**读 / 写 paper.tex; 仅写 `scored_<ts>.json` 和 `halt_decision.json` 到 `out/`. 通用化合规 ([[feedback_paper_agent_long_term_generality]])：所有字段 CLI 参数化；anchors / halt 算法跨学科可消费；测试用 non_aphasia_cs fixture 验证 paper_id 注入路径.
