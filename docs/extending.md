...
## score 子包：红队反诘 + Ensemble（0.1.1.dev M-E）

`score/red_team.py` + `score/ensemble.py` 把 spec §D.4 的红队反诘 prompt 池、`num_reviews_ensemble`、Krippendorff α self-reliability 与 Area Chair aggregate 全部固化。M-E 仍**不接真 LLM**——所有 LLM 调用通过 `evaluator` hook 注入，便于 M-F 平滑接入后端。

### 红队 7 个 failure mode (`score/red_team.py`)

`DEFAULT_FAILURE_MODES` 7 元 tuple 顺序锁死（spec §D.4，PaperOrchestra MIT red-team contract）：

| mode | 默认 severity | detector_regex |
|------|--------------|----------------|
| `factual_fabrication` | ERROR | — (LLM-only) |
| `methodology_handwave` | WARN | — (LLM-only) |
| `selective_reporting` | WARN | — (LLM-only) |
| `citation_inflation` | WARN | — (LLM-only) |
| `ethics_omission` | ERROR | — (LLM-only) |
| `stat_misuse` | WARN | — (LLM-only) |
| `scope_overclaim` | WARN | 英文 + 中文社工诱导词 (`obviously / clearly demonstrates / trivially / 显然.{0,8}(?:证明\|表明) / 毫无疑问`) |

- **有 `detector_regex` 的 mode** 在不依赖 LLM 时即可命中 obvious 案例（仅扫非 `%`-注释行）
- **detector=None 的 mode** 在 `evaluator=None` 时**静默 skip**；在 M-F 注入真 LLM evaluator 后才返回 finding。`evaluator` 接口契约：`(mode, tex_text, paper_id) -> list[RedTeamFinding]`

### safe-revision 白名单

`SAFE_REVISION_RE` 让 edit_gate Layer 3 区分"安全可应用 revision"vs"需人工评估"的改动：

```
<action> <scope>: <content>
   action ∈ {replace, delete, reword}   (case-insensitive)
   scope  ∈ {phrase,  sentence, clause}
```

例：`replace phrase: foo with bar` → safe；`just rewrite the whole paragraph` → not safe。`is_safe_revision(s)` 返回 bool。

CLI:
```bash
python -m paper_agent.score.red_team \
    --tex paper.tex --paper-id <id> \
    --modes scope_overclaim ethics_omission \
    --out out/red_team.json
```

### Ensemble + Krippendorff α (`score/ensemble.py`)

**核心 API：**

| 函数 | 用途 |
|------|------|
| `num_reviews_ensemble(evaluator, paper_path, *, n_runs, paper_id)` | 反复跑 evaluator 收集 N 个 `Scored` |
| `krippendorff_alpha_interval(units)` | 纯数学 interval-level α (Hayes & Krippendorff 2007) |
| `ensemble_alpha(runs)` | 把 7 维当 unit / 每 run 1 rating → single α |
| `area_chair_aggregate(runs, *, paper_id, scored_at)` | 终值 `Scored`：每维 median + α 标注 + evidence 去重 + red_team 合并 |

**Krippendorff α 退化约定：**
- 全体观察值完全相同（Do=De=0）→ α = 1.0
- 所有 unit 只有 1 rating 或空输入 → α = None
- α 计算后透过 `derive_confidence(alpha)` 映射 `high / low / fail`（[[复用 M-D dimensions.py]]）

**evaluator 接口契约：**
```python
def evaluator(paper_path: Path, paper_id: str, run_idx: int) -> Scored: ...
```
M-E 测试用 stub evaluator（deterministic Scored）；M-F 接入真 LLM 后端时**只换 evaluator，不动 ensemble.py**。

CLI:
```bash
python -m paper_agent.score.ensemble \
    --runs-dir out/runs \
    --paper-id <id> \
...
- 7 failure mode + α 算法 + Area Chair aggregation 全 paper-agnostic（无学科 / 语言 hardcode）
- 红队 prompt 模板和 detector regex 同时含英文 + 中文社工诱导词，但**不**绑定任何具体论文 / 学科
- `paper_id` 完全 CLI / 调用方参数化；`area_chair_aggregate` 的 `paper_id` 参数明确 override 各 run 中的 id
- L-033 read-only：M-E 全模块**永不读写** `paper.tex`（red_team 只读，ensemble 只接受 `Scored` 内存对象）
- 测试用 non_aphasia_cs fixture（CS / ANN 论文）验证 paper-agnostic 跑通
